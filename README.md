...
* **Multi-threading** is supported via `--cores N`, which runs N worker threads with a PROXY v2 dispatcher. Each core gets its own event loop and V8 isolates. On Linux, cores are pinned to CPUs.
